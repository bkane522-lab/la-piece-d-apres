  ...
  return (
    <main className="app-page project-detail">
      ...
      <section className="dashboard-main">
        ...
        {tab === "overview" && <OverviewTab project={project} onCapture={() => setTab("capture")} />}
...
function OverviewTab({ project, onCapture }: { project: ProjectRow; onCapture: () => void }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const s = getSupabaseBrowserClient();
        const { data } = await s.from("project_files")
          .select("category,caption")
          .eq("project_id", project.id)
          .eq("file_type", "image");
        setPhotos((data || []).filter(f => f.category).map(f => {
          const decoded = decodeCaption(f.caption);
          return { shotType: f.category as string, tag: decoded.tag, note: decoded.note };
        }));
      } finally {
        setLoading(false);
      }
  ...
  const keep = photos.filter(p => p.tag === "garde");
  const change = photos.filter(p => p.tag === "change");
  const unsure = photos.filter(p => p.tag === "ne_sais_pas");
  const notes = photos.filter(p => p.note.trim()).slice(0, 3);
  const progress = Math.min(100, Math.round((photos.length / SHOT_TYPES.length) * 100));
  const shotLabel = (key: string) => SHOT_TYPES.find(s => s.key === key)?.label || key;

  return (
    <div className="overview-smart">
      <div className="overview-smart__hero">
        <div>
          <p className="eyebrow">Dossier intelligent</p>
          <h2>Votre pièce en un coup d’œil</h2>
          <p>{loading ? "Analyse du dossier…" : photos.length ? `${photos.length} photo${photos.length > 1 ? "s" : ""} structurée${photos.length > 1 ? "s" : ""} pour préparer la suite.` : "Commencez par photographier votre pièce."}</p>
        </div>
        <div className="overview-smart__progress" aria-label={`${progress}% du parcours photo`}>
          <strong>{progress}%</strong>
          <span>capture</span>
        </div>
      </div>

      <div className="overview-smart__stats">
        <div><strong>{photos.length}</strong><span>Photos</span></div>
        <div><strong>{keep.length}</strong><span>À garder</span></div>
        <div><strong>{change.length}</strong><span>À changer</span></div>
        <div><strong>{unsure.length}</strong><span>À décider</span></div>
      </div>

      {(keep.length > 0 || change.length > 0 || unsure.length > 0) && (
        <div className="overview-smart__decisions">
          {keep.length > 0 && <section><span className="overview-smart__dot overview-smart__dot--keep" /> <strong>À conserver</strong><p>{keep.map(p => shotLabel(p.shotType)).join(" · ")}</p></section>}
          {change.length > 0 && <section><span className="overview-smart__dot overview-smart__dot--change" /> <strong>À transformer</strong><p>{change.map(p => shotLabel(p.shotType)).join(" · ")}</p></section>}
          {unsure.length > 0 && <section><span className="overview-smart__dot overview-smart__dot--unsure" /> <strong>À décider</strong><p>{unsure.map(p => shotLabel(p.shotType)).join(" · ")}</p></section>}
        </div>
      )}

      {notes.length > 0 && (
        <div className="overview-smart__notes">
          <p className="eyebrow">Notes principales</p>
          {notes.map((n, i) => <p key={`${n.shotType}-${i}`}><strong>{shotLabel(n.shotType)}</strong> — {n.note}</p>)}
        </div>
      )}

      {photos.length === 0 && !loading && (
        <button className="button-primary" onClick={onCapture}>Photographier ma pièce</button>
      )}
    </div>
  );
}
